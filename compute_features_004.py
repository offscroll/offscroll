#!/usr/bin/env python3
"""
Compute layout features for graded spreads — n=200 final set (Task #343).

Merges grades from batches 2, 3, 4, and 5 (50 spreads each, 200 total)
and re-runs the feature pipeline from compute_features_003.py over the
combined set. Schema is identical to features-003.csv.

Train/val split: 80/20 with seed 42 on the merged 200 spreads. The
...
Note: batch-005.csv has a trailing 'COMPLETED' row left over from a
runner artifact. The loader filters it out (spread_id must start with
's-' and technical/style must be parseable integers).
"""
...
OUTPUT_PATH = f"{BASE}/features/features-004.csv"
...
# ─────────────────────────────────────────────────────────────
# Helpers (unchanged from compute_features_003.py)
# ─────────────────────────────────────────────────────────────
...
for path in GRADES_PATHS:
    ...
    with open(path) as f:
        ...
        for row in reader:
            ...
            # Filter out non-spread rows (e.g., trailing "COMPLETED" artifact)
...
# ─────────────────────────────────────────────────────────────
# Train/val split — fresh 80/20 with seed 42 over the 200 spreads.
...
for spread_id in spread_ids:
    ...
    if not edition_id:
        # Fall back: derive edition_id from spread_id format `s-{nnn}-{idx}`.
        # edition-map.json is missing terminal spreads from some editions
        # (e.g. training-090 has 23 spreads in metadata but only 20 in the map).
        # The spreads exist in metadata.json, so we recover them here.
