...
**Date:** 2026-04-02 (Task #238)
**Source data:** `training/grades/batch-001.csv` (50 graded spreads)
**Output:** `training/features/features.csv`
...
**Batch 1 quality note:** Neville's batch-001-summary.md reports that
~50% of pages in this batch have the `}{` rendering bug (empty pages).
The grades in this batch largely measure "does the page have content"
rather than "is the composition good." Features computed from edition
metadata will not capture this bug — they describe the content
*intended* for the pages, not what was actually rendered. Predictive
power from batch 1 will be low; the feature engineering is correct but
the training signal is poor. This is expected and documented.
...
| Column | Type | Description |
|--------|------|-------------|
| `technical_grade` | int 1–10 | Scale A: technical proficiency. Measures composition errors, column balance, spacing, fill, and image placement. Median = 2 in batch 1 (system has fundamental rendering problems). |
| `style_grade` | int 1–10 | Scale B: editorial style. Measures visual rhythm, surprise calibration, front-page impact, and section transitions. Median = 2 in batch 1. |
...
| `edition_image_count_total` | int | Total number of embedded images across the edition. Batch 1 note: nearly all items have 0 images (image pipeline appears broken for training data). |
...
| `est_word_count_mean` | float | `est_word_count / est_item_count`. Average item length on the spread. High variance spreads have mixed short and long items. |
...
| `est_image_count` | int | Estimated number of images on the spread pages. From edition.json images arrays; batch 1 note: nearly all zero. |
...
|--------|------|-------------|------------|
| `d3_image_fraction` | float [0,∞) | Images per item on estimated spread pages (`est_image_count / est_item_count`). Proxy for D3 (image ratio). **Batch 1 note:** nearly all zero — image pipeline broken. | Does not capture image size or position; those require rendered output. |
| `d5_fill_fraction` | float | Estimated words on spread / (WORDS_PER_PAGE_CAPACITY × n_pages). Capacity constant = 450 words/page (rough estimate for 2–3 column newspaper layout). Values > 1.0 indicate estimated overload; values < 0.5 indicate sparse content. Proxy for D7 (page fill). | Capacity estimate is a fixed constant, not calibrated to individual editions. Underestimates fill for brief-heavy editions (many short items with whitespace); overestimates for long-form editions. |
| `d7_template_entropy` | float ≥ 0 | Shannon entropy of `layout_hint` distribution across estimated spread items. 0 = all same template; 1 = equal brief/standard mix. Proxy for template diversity (D7 dimension). | Uses estimated item set; accuracy depends on item placement estimate. |
| `d8_word_count_cv` | float ≥ 0 | Coefficient of variation of word counts across estimated spread items. High CV = items vary widely in length → harder to achieve uniform spacing (D8). | Word count variation is a proxy for height variation, not a direct measure. Actual height depends on template, headline length, and line wrapping. |
| `anchor_strength` | float ≥ 1 | Max word count / mean word count across estimated spread items. 1.0 = all items same length; higher = one dominant long item. Proxy for anchor strength (max area / mean area). | Same as d8: word count is a proxy for rendered area. |

---

## Features Requiring Rendered Output (NA in Batch 1)
...
**Note:** Batch 1 grades are dominated by the `}{` rendering bug
(technical grade 2 on ~50% of spreads). The train/val split may not
be stratified by grade — the small batch size and grade distribution
make stratification impractical. Consider stratified splitting when
batch 2 grades are added.
...
n_spreads:               50
train / val:             40 / 10
technical_grade mean:    3.0   (median 2, std 1.7)
style_grade mean:        2.6   (median 2, std 1.2)
d5_fill_fraction:        min=0.31  max=3.36  mean=1.55
d7_template_entropy:     min=0.00  max=1.00  mean=0.17
d8_word_count_cv:        min=0.00  max=1.97  mean=0.58
anchor_strength:         min=1.00  max=7.11  mean=1.92
est_items_per_page:      min=0.50  max=11.0  mean=1.77
est_words_per_page:      min=138   max=1512  mean=699
page_position_frac:      min=0.00  max=1.00  mean=0.53
```
