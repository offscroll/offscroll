"""
Definitive go/no-go gate at n=200 checkpoint (features-004.csv).
Ada — Task #344 (final checkpoint of #311/#330/#342 series).

At n=200 with 18 reduced features, p/n = 0.09 — well within
statistical comfort. If the model doesn't clear R² > 0.5 here,
it's a real signal about feature limitations, not sample size.
"""
...
features_path = f'{BASE}/features/features-004.csv'

df = pd.read_csv(features_path)
print(f"Loaded {len(df)} spreads from features-004.csv")
...
# --- Redundancy removal (same rules as #330/#342) ---
print("\n" + "="*60)
print("REDUNDANCY REMOVAL (same rules as #330/#342)")
print("="*60)
...
# ============================================================
# 1. TECHNICAL MODEL — full alpha sweep
# ============================================================
print("\n" + "="*60)
...
# Headline: alpha=10 (specified)
ridge_tech10 = Ridge(alpha=10.0)
ridge_tech10.fit(X_train, y_tech_train)
...
# Tuned model: alpha=0.1 (best from #342 sweep)
ridge_tech01 = Ridge(alpha=0.1)
ridge_tech01.fit(X_train, y_tech_train)
...
print(f"Ridge (alpha=0.1): Train R²={r2_tech_ridge01_train:.4f} MAE={mae_tech_ridge01_train:.4f} | "
...
ridge_style10 = Ridge(alpha=10.0)
ridge_style10.fit(X_train, y_style_train)
...
ridge_style01 = Ridge(alpha=0.1)
ridge_style01.fit(X_train, y_style_train)
...
print(f"Ridge (alpha=0.1): Train R²={r2_style_ridge01_train:.4f} MAE={mae_style_ridge01_train:.4f} | "
...
for idx in tech_resid_order[:10]:
    ...
    print(f"    {sid}: actual={a}, predicted={p:.2f}, residual={r:.2f}")
    top_resids_tech.append({'spread_id': sid, 'actual': float(a), 'predicted': float(p), 'residual': float(r)})
...
# Pred vs actual scatter (4 panels) — using alpha=0.1 for ridge panels
...
plt.tight_layout()
plt.savefig(f'{BASE}/models/pred_vs_actual_004.png', dpi=150)
plt.close()
print("Saved: pred_vs_actual_004.png")
...
plt.colorbar(im); plt.title('Feature Correlation (reduced, batch-004)')
plt.tight_layout()
plt.savefig(f'{BASE}/models/feature_correlation_004.png', dpi=150)
plt.close()
print("Saved: feature_correlation_004.png")
...
# Bootstrap CIs (alpha=0.1, our preferred operating point)
...
print(f"  Tech (alpha=0.1): median={np.median(boot_t01):.3f}, 95% CI=[{np.percentile(boot_t01,2.5):.3f}, {np.percentile(boot_t01,97.5):.3f}]")
print(f"  Style (alpha=0.1): median={np.median(boot_s01):.3f}, 95% CI=[{np.percentile(boot_s01,2.5):.3f}, {np.percentile(boot_s01,97.5):.3f}]")

# Also bootstrap at alpha=10 for #342 comparability
np.random.seed(42)
boot_t, boot_s = [], []
for _ in range(1000):
    idx = np.random.choice(len(X_all), size=len(X_all), replace=True)
...
print(f"  Tech (alpha=10):  median={np.median(boot_t):.3f}, 95% CI=[{np.percentile(boot_t,2.5):.3f}, {np.percentile(boot_t,97.5):.3f}]")
print(f"  Style (alpha=10): median={np.median(boot_s):.3f}, 95% CI=[{np.percentile(boot_s,2.5):.3f}, {np.percentile(boot_s,97.5):.3f}]")
...
# Learning curve — fixed val=40 at n=200, vary n_train ∈ {40,80,120,160,200}
# Note: at n=200, we have ~160 train + ~40 val. We use 5-fold CV for the n=200 point.
print("\n--- Learning curve (Ridge alpha=0.1) — fixed val set ---")
np.random.seed(42)
n_targets = [40, 80, 120, 160]
lc_results_tech = []
for n in n_targets:
    if n > len(X_train):
        continue
    ...
    for _ in range(50):
        ...
        rl = Ridge(alpha=0.1).fit(X_train[idx], y_tech_train[idx])
    ...
    lc_results_tech.append({'n_train': n, 'val_r2_mean': m, 'val_r2_std': s})
# n=200 via 5-fold CV
lc_results_tech.append({'n_train': 200, 'val_r2_mean': cv_t01_m, 'val_r2_std': cv_t01_s, 'method': '5-fold CV'})
print(f"  n_total=200 (5-fold CV): Val R² = {cv_t01_m:.3f} +/- {cv_t01_s:.3f}")

# Style learning curve
print("\n--- Learning curve — STYLE (Ridge alpha=0.1) ---")
np.random.seed(42)
lc_results_style = []
for n in n_targets:
    if n > len(X_train):
    ...
    for _ in range(50):
        idx = np.random.choice(len(X_train), size=n, replace=False)
        rl = Ridge(alpha=0.1).fit(X_train[idx], y_style_train[idx])
        scores.append(r2_score(y_style_val, rl.predict(X_val)))
    m = float(np.mean(scores)); s = float(np.std(scores))
    print(f"  n_train={n:3d}: Val R² = {m:.3f} +/- {s:.3f}")
    lc_results_style.append({'n_train': n, 'val_r2_mean': m, 'val_r2_std': s})
lc_results_style.append({'n_train': 200, 'val_r2_mean': cv_s01_m, 'val_r2_std': cv_s01_s, 'method': '5-fold CV'})
print(f"  n_total=200 (5-fold CV): Val R² = {cv_s01_m:.3f} +/- {cv_s01_s:.3f}")

# Plot learning curve
fig, ax = plt.subplots(1, 1, figsize=(9, 6))
ns = [r['n_train'] for r in lc_results_tech if 'method' not in r]
ms = [r['val_r2_mean'] for r in lc_results_tech if 'method' not in r]
ss = [r['val_r2_std'] for r in lc_results_tech if 'method' not in r]
ax.errorbar(ns, ms, yerr=ss, marker='o', label='Technical (n_val=40 fixed)', linewidth=2, color='C0')
ax.scatter([200], [cv_t01_m], marker='*', s=200, color='C0',
           label=f'Tech 5-fold CV @ n=200 ({cv_t01_m:.3f})', zorder=5)

ns_s = [r['n_train'] for r in lc_results_style if 'method' not in r]
ms_s = [r['val_r2_mean'] for r in lc_results_style if 'method' not in r]
ss_s = [r['val_r2_std'] for r in lc_results_style if 'method' not in r]
ax.errorbar(ns_s, ms_s, yerr=ss_s, marker='s', label='Style (n_val=40 fixed)', linewidth=2, color='C1')
ax.scatter([200], [cv_s01_m], marker='*', s=200, color='C1',
           label=f'Style 5-fold CV @ n=200 ({cv_s01_m:.3f})', zorder=5)

# Reference: prior endpoints
ax.scatter([40], [0.32], marker='D', s=120, color='black',
           label='#330 endpoint (n=50)', zorder=5)
ax.scatter([120], [0.51], marker='D', s=120, color='gray',
           label='#342 endpoint (n=150 5-fold CV)', zorder=5)

ax.axhline(0.5, color='green', linestyle=':', alpha=0.5, label='GO threshold (R²=0.5)')
ax.axhline(0.4, color='gold', linestyle=':', alpha=0.5, label='Qualified GO (R²=0.4)')
...
ax.set_title('Learning Curve — n=200 definitive checkpoint (#344)')
ax.legend(loc='lower right', fontsize=8)
ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(f'{BASE}/models/learning_curve_004.png', dpi=150)
plt.close()
print("Saved: learning_curve_004.png")

# ============================================================
# SAVE RESULTS
# ============================================================
results = {
    'task': '#344 (n=200 definitive checkpoint)',
    ...
    'technical_model': {
        'ridge_alpha10': {'train_r2': float(r2_tech_ridge_train), 'val_r2': float(r2_tech_ridge_val),
                          'train_mae': float(mae_tech_ridge_train), 'val_mae': float(mae_tech_ridge_val)},
        'ridge_alpha01': {'train_r2': float(r2_tech_ridge01_train), 'val_r2': float(r2_tech_ridge01_val),
    ...
    'style_model': {
        'ridge_alpha10': {'train_r2': float(r2_style_ridge_train), 'val_r2': float(r2_style_ridge_val),
                          'train_mae': float(mae_style_ridge_train), 'val_mae': float(mae_style_ridge_val)},
        'ridge_alpha01': {'train_r2': float(r2_style_ridge01_train), 'val_r2': float(r2_style_ridge01_val),
    ...
    'learning_curve_technical': lc_results_tech,
...
with open(f'{BASE}/models/results_batch004.json', 'w') as f:
    json.dump(results, f, indent=2, default=str)
print("Saved: results_batch004.json")
print("\nDone.")
